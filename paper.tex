...
The \ff{malloc} object is also support resizing via \adeff{resized} attribute and copying the data withing
...
\begin{itemize}
    ...
    \item \deff{repeated}: repeates itself given amount of \(times\)
...
The object \deff{map} is a decorator of \ff{tuple} of \deff{map.enty} objects which are pairs \((k, v)\).
...
The atribute \deff{all-lines} reads all lines until it's possible.
...
The object \deff{tmpdir} is an abstraction of temprorary directory and is decorator of \deff{dir}.
...
\begin{itemize}
    ...
    \item \deff{recv}: recieve \(x\) amount of bytes from socket
