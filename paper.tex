...
\usepackage[maxnames=1,minnames=1,maxbibnames=100,natbib=true,citestyle=authoryear,bibstyle=authoryear,doi=false,url=false,isbn=false,isbn=false,backend=bibtex]{biblatex}
...
\newcommand\deff[1]{\textbf{\ff{#1}}}
\newcommand\adeff[1]{\textbf{\aff{#1}}}
...
An object in EO is either a composition of other objects or an atom, which is a foreign function interface to a lower-level runtime such as, for example, JVM or Assembly. In this paper, we denote atoms by a star, for example: \aff{mul}.
...
\begin{itemize}
    \item As fewer atoms as possible.
    Whenever it's possible, we try to implement objects in EO, instead of turning them into atoms.
    \item No duplication.
    There should be no functionality implemented two or more times in the entire taxonomy.
\end{itemize}

In case of any runtime error, an object is supposed to ``return'' an error object, with \ff{msg} attribute present (as \ff{string}) and \ff{@} attribute absent.
...
The attributes \adeff{mul} and \adeff{div} are a multiplication and a division of this number and another \ff{int}.
...
The attributes \adeff{mul} and \adeff{div} are a multiplication and a division of this number and another \ff{float}.
The attribute \deff{neg} is the same number with a different sign.
...
The object \adeff{seq} is an abstraction of sequence of objects to be dataized sequentially irrespective of the dataization results. For example, this code prints two messages to the console and makes a division by zero in the middle:
...
\begin{itemize}
    ...
    \item \deff{ltrim}: removes leading
    \item \deff{replace}: finds and replaces a sub-string
    \item \deff{rtrim}: removes trailing spaces
...
\tbd{Maybe other functions?}

\tbd{Regular expressions}
...
The attributes \deff{each}, \deff{reduce}, \deff{find}, \deff{filter}, and \deff{map} are similar to \ff{forEach}, \ff{reduce}, \ff{find}, \ff{reduce}, and \ff{map} methods of \ff{Array} object in JavaScript~\citep{EcmaScript}. The attributes \deff{eachi}, \deff{reducei}, \deff{filteri}, \deff{findi}, and \deff{mapi} are semantically the same, but with an \ff{int} as a counter of a cycle.
...
The attributes \ff{with}, \ff{without}, \ff{find}, and \ff{findi} are reimplemented in \ff{map}.
...
The attributes \ff{with}, \ff{without}, \ff{find}, and \ff{findi} are reimplemented in \ff{set}.

\subsection{Mutable Arrays}

The objects \deff{array-list} and \deff{linked-list} are the abstractions of mutable lists that behave like \ff{array}. They have three additional attributes: \deff{inject}, \deff{replace}, and \deff{extract}.

\tbd{describe them better}
