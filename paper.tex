...
The \adeff{memory} object is an abstraction of a read-write memory capable of storing one of five primitive data objects mentioned in
\cref{sec:primitives}.
The \adeff{write} attribute implements writing operation, while the \aff{memory} object itself decorates the data object stored in it.
...
The \deff{random} object is a abstraction of a random \ff{number} between zero
and one inclusively. It has an additional attribute \adeff{pseudo} that
makes \ff{random} with a pseudorandom seed.

The \deff{angle} object is a decorator of \ff{number}. It assumes that the angle
...
The \deff{number} object is a decorator of \ff{int} and \ff{number} with the
