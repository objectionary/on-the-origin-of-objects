...
We tend to minimize the global scope, keeping the number
...
We therefore select global objects carefully,
  ...
  favoring \textbf{preciseness over expressiveness}.

Wherever possible, objects must be implemented in \eolang{},
...
We therefore favor \textbf{transparency over speed}.

There should be no functionality implemented
...
We therefore favor \textbf{reusability over complexity}.

Object names must not be abbreviated and must constitute
...
\end{ffcode}
%
This expression means ``\(x\) times \(y\) is \(z\).''
There are a few exceptions though, like \adeff{eq}, \deff{seq}, \deff{lt},
  and a few others.
...
The \deff{bool} object abstracts a boolean value, parameterized by
  its \deff{if} attribute---a two-argument object that returns either
...
For example, the number \(42\) as a 64-bit integer always occupies
  exactly eight bytes, regardless of its value:

\begin{ffcode}
42.as-i64.as-bytes
\end{ffcode}

The result is \eohex{00-00-00-00-00-00-00-2A}.
