...
\begin{itemize}
    ...
    for example \ff{x.times.y > z} means ``$x$ times $y$ is $z$.'' There are a few exceptions though, like \ff{eq}, \ff{seq}, \ff{lt}, and a few others.
\end{itemize}
...
The attributes \deff{each}, \deff{reduced}, \deff{found}, \deff{filtered}, and \deff{mapped} are similar to \ff{forEach}, \ff{reduce}, \ff{find}, \ff{reduce}, and \ff{map} methods of \ff{Array} object in JavaScript~\citep{EcmaScript}. The attributes \deff{eachi}, \deff{reducedi}, \deff{filteredi}, \deff{foundi}, and \deff{mappedi} are semantically the same, but with an \ff{int} as a counter of a cycle.
