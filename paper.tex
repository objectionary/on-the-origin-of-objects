...
object, with the \ff{message} attribute present (as a \ff{string}) and
...
and
\adeff{as-string}.
The attribute \aff{as-int} expects the size of the sequence to be less or
equal to eight. The attribute \aff{as-bool} is \ff{FALSE} only if all bytes
equal to zero. The attribute \aff{as-float} expects exactly eight bytes.
...
The \adeff{chained} attribute is a new sequence, which is a concatenation of two byte sequences: the current and the provided one.
...
The \adeff{lt} attribute is a \ff{bool} object \ff{TRUE} if it's ``less than'' another \ff{int} and \ff{FALSE} otherwise. Attributes \deff{gt}, \deff{lte}, and \deff{gte} are ``greater than,'' ``less than or equal,'' and ``greater than or equal'' comparisons respectively.
...
The \deff{bool} object is a one-bit abstraction of a Boolean value. Its \aff{as-bytes} is either \eohex{00-} or \eohex{01-}.

There are two pre-defined objects \deff{TRUE} and \deff{FALSE}.

The \deff{not} attribute is a Boolean inversion. Attributes \deff{and}, \deff{or}, \deff{xor} are logical conjunction, disjunction, and exclusive disjunction of the object itself with an a array of other Boolean values; they use \aff{and}, \aff{or}, and \aff{xor} from \ff{\^{}.as-bytes} respectively.
...
The \adeff{lt} attribute is a \ff{bool} object \ff{TRUE} if it's ``less than'' another \ff{float} and \ff{FALSE} otherwise. Attributes \deff{gt}, \deff{lte}, and \deff{gte} are ``greater than,'' ``less than or equal,'' and ``greater than or equal'' comparisons respectively.
...
it's associated index number---positive one, zero indexed and starting
...
The \adeff{chained} attribute is a new \ff{tuple}, which is
a concatenation of two tuples: the current and the provided one.

All other manipulations with tuples can be implemented in their
decorators. Some of them are explained in \cref{sec:collections}.
...
condition as \ff{bool} and an abstract object with one free attribute. When
being dataized, the object \aff{while} first dataizes the condition and then
either stops if it is \ff{FALSE} or dataizes the abstract object with an
\ff{int} equal to the number of the iterating cycle. This code makes a few
...
The \deff{nop} object (stands for ``No Operations'') encapsulates everything,
doesn't dataize it, and is always \ff{TRUE}:
...
Objects \deff{int8}, \deff{int16}, \deff{int32}, and \deff{int128} are decorators
of \ff{bytes} with a predefined size. They implement the same numeric operations
as \ff{int}.
Objects \deff{float16}, \deff{float32}, and \deff{float128} are
...
The \adeff{cage} object is similar to \aff{memory}, but it can store any object,
not only five primitives. The use of \aff{cage} is strongly discouraged. The
...
The \deff{heap} object is an abstraction of a random-access memory of certain
size. It behaves like \ff{bytes}, but has an additional attribute
\adeff{write}. In this example, one kilobyte of memory with random data is allocated
when a copy of \ff{heap} is created, a non-copying slice \ff{s} of 13 bytes is
made starting with the 200-th byte, data is written into it, then a non-copying
sub-slice is created of only five bytes, and finally a string is read back
and \ff{"Hello"} is printed:
...
The object \deff{random} is a abstraction of a random \ff{float} between zero
and one inclusively. It has an additional attribute \adeff{pseudo} that
makes \ff{random} with a pseudorandom seed.

The object \deff{angle} is a decorator of \ff{float}. It assumes that the angle
is in radians and has the following attributes for trigonometric functions:
\deff{sine}, \deff{cosine}, and \deff{tangent}. There are attributes \deff{as-degrees}
and \deff{as-radians} for switching from radians to degrees and backwards.
...
The \deff{sscanf} object encapsulates a format \ff{string} and
a content \ff{string}. It behaves like a tuple of scanned data objects.
...
Here we discuss abstractions of lists, maps, sets, and other ``collections.''
All objects presented in this Section belong to the \ff{QQ.collections} package.
...
to \ff{QQ.net} package.

The \deff{socket} object is an abstraction of a TCP socket. The attribute
\adeff{connect} establishes a connection and makes a copy of the socket object. Then,
its attributes \adeff{as-input} and \adeff{as-output} may be used for reading
...
Here we define objects used for parsing and printing of HTTP requests and
